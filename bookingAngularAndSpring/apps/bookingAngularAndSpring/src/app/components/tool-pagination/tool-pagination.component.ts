...
export class ToolPaginationComponent implements AfterContentChecked {
  ...
  setUp() {
    ...
    this.pagination = Math.ceil(
      this.totalItems / this.table.pagination.itemPerPage
    );
    this.paginaCorrente =
      Math.floor(this.start / this.table.pagination.itemPerPage) + 1;
  ...
  range(numero: number): number[] {
    let arrayNum: number[] = [];
    for (let i = 1; i <= numero; i++) arrayNum.push(i);
    return arrayNum;
  }
  ...
  ngAfterContentChecked(): void {
    if (
      this.end - this.start !=
      parseInt(String(this.table.pagination.itemPerPage))
    ) {
      this.table.pagination.itemPerPage = parseInt(
        String(this.table.pagination.itemPerPage)
      );
      this.setUp();
      this.emit.emit(this.paginaCorrente);
    }
    if (!this.pagination) this.setUp();
  }
}
